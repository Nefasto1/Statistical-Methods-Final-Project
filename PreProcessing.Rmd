...
tag <- c(0, quantile(df$popularity, 0.25), quantile(df$popularity, 0.75), Inf)
...
### Default
```{r}
lm_model <- lm(best_price ~ brand_name + os + phone_type + sellers_amount + screen_size + memory_size + battery_size + month + year, data = train)
summary(lm_model)
...
fit_rf <- randomForest(best_price ~ brand_name + os + phone_type + screen_size + memory_size + battery_size + month + year, data = train)
fit_rf
