...
df <- read.csv("C:/Users/Utente/Desktop/STATISTICAL MODELS prog/phones_data.csv", header=T)
str(df)
...
pairs(df[c(-1, -2, -3, -4, -13)], pch=19, lower.panel=panel.smooth)
...
corr <- cor(df[, c(-1, -2, -3, -4, -13)], use="complete.obs" )
...
df$month <- sapply(df$release_date, FUN = function(x) {strsplit(x, split = '[-]')[[1]][1]})
df$year <- sapply(df$release_date, FUN = function(x) {strsplit(x, split = '[-]')[[1]][2]})
