...
df$log_memory_size <- log2(df$memory_size)
...
for (i in bimodal_vars) {
  p1 <- ggplot(df, aes_string(x=i, fill = "os")) +
    geom_histogram(color = "black", bins = 12) + ggtitle(toupper(i)) + theme(plot.title = element_text(hjust = 0.5, size= 12, face="bold.italic")) + scale_fill_manual(values = c("yellow", "purple","purple"))
    plots <- c(plots, list(p1))
    }
...
for (i in cat_variables) {
  p1 <- ggplot(df, aes_string(x= factor(df[,i]), y = df$best_price)) + geom_boxplot(fill="steelblue") + labs(x = i, y = "best_price") + theme(plot.title = element_text(hjust = 0.5, size= 12, face="bold.italic")) + ggtitle(toupper(i))
    plots <- c(plots, list(p1))}
...
```{r}
start <- Sys.time()
fit_gam2 <- gam(best_price ~ os +  screen_size + memory_size + brand_name , data = train)
end <- Sys.time()
summary(fit_gam2)
...
start <- Sys.time()
fit_gam3 <- gam(best_price ~ os + s(screen_size) + s(memory_size) + s(battery_size) + s(sellers_amount) + s(month)+  popularity_levels + brand_name, data = train, method = "REML")
end <- Sys.time()
summary(fit_gam3)
...
```
We can remove the non significant variables from the model, in this case `popularity_levels`. Regarding the splines we can see that the `sellers_amount` and `month` variables have a linear effect on the model, so we can remove the spline terms from them.

TODO: controllare se ha senso la giustificazione
```{r}
start <- Sys.time()
fit_gam4 <- gam(best_price ~ os + s(screen_size) + s(memory_size) + s(battery_size) + sellers_amount+ brand_name, data = train, method = "REML")
end <- Sys.time()
summary(fit_gam4)
...
```
By removing the splines from the two variables, we notice that `sellers_amount` is relevant for the model, while `month` is not, therefore we don't consider it .
...
plot(fit_gam4)
```
The smooth terms for `screen_size`,`battery_size` and `memory_size` show a clear non-linear relationship with the `best_price` variable, as confirmed by the edf values in the summary of the model.
```{r}
par(mfrow=c(2,2))
gam.check(fit_gam4)
```
TODO: commentare i plot sei residui ---> vengono male, come sistemarli?
può andare bene la trasformazione in log?

```{r}
plot(fit_gam4, residuals = TRUE, pch = 16, cex = 0.7, col = "steelblue")
```
...
```{r}
start <- Sys.time()
fit_gam5 <- gam(best_price ~ os + s(screen_size) + s(memory_size) +sellers_amount  + brand_name +  os:memory_size + os:battery_size  , data = train, method = "REML")
end <- Sys.time()
summary(fit_gam5)

y_hat <- predict(fit_gam5, newdata = train)
pred <- predict(fit_gam5, newdata = test)
results5 <- c("RMSE"=RMSE(y_hat, train$best_price), "R2"=R2(y_hat, train$best_price), "RMSE_test"=RMSE(pred, test$best_price), "R2_test"=R2(pred, test$best_price), "AIC"=AIC(fit_gam5), "BIC"= BIC(fit_gam5), "GCV"=GCV(fit_gam5), "Time"=end-start)
```
```{r}
par(mfrow=c(1,2))
plot(fit_gam5)
...
```
TODO: i plot dei residui però mostrano che c'è un comportamneto strano... COme sistemare, cosa si evince da questi plot?

### Add splines to interactions
```{r}
start <- Sys.time()
fit_gam6 <- gam(best_price ~ os + s(screen_size) + s(memory_size) + sellers_amount  + brand_name +  s(memory_size, by=os) + s(battery_size, by=os)  , data = train, method = "REML")
end <- Sys.time()
summary(fit_gam6)
...
results6 <- c("RMSE"=RMSE(y_hat, train$best_price), "R2"=R2(y_hat, train$best_price), "RMSE_test"=RMSE(pred, test$best_price), "R2_test"=R2(pred, test$best_price), "AIC"=AIC(fit_gam6), "BIC"= BIC(fit_gam6), "GCV"=GCV(fit_gam6), "Time"=end-start)
```
```{r}
par(mfrow=c(2,2))
plot(fit_gam6)
```
TODO: io non metterei gli splines sulle interazioni, non mi sembrano significative sia dai plot che dai edf
toglierei l'ultimo modello completamente
- da esplorare la questione della trasformazione al logaritmo, effettivamente a questo punto avreebbe senso fare una spline, se abbiamo in qualche modo "linearizzato" la relazione tra la variabile e la risposta?
Potrebbe essere il motivo per cui i  plot dei modelli precedentemente analizzati vengono male... Sembrano quasi tutti lineari... Ricontrollare
Valori AIC molto alti perchè?

```{r}
gam_results <- rbind(results1, results2, results3, results4, results5, results6)
rownames(gam_results) <- c("All variables", "Without non significant variables", "With splines", "Without non significant splines", "With interactions", "With splines on interactions")
gam_results
```
